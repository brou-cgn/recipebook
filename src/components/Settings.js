...
const DARK_MODE_ICON_ROWS = [
  ...
  { key: 'nutritionManualSave', label: 'Nährwertemodal: Menge speichern' },
  ...
  { key: 'menuPhotoUpload', label: 'Foto hochladen (Menü bearbeiten)' },
...
function Settings({ onBack, currentUser, allUsers = [], allRecipes = [], onUpdateRecipe }) {
  ...
  const handleSave = async () => {
    try {
      ...
      // Force a fresh Firestore fetch on the next getSettings() call (in-memory
      // and localStorage caches would otherwise keep serving stale lists, e.g.
      // newly added Getränke-/Gebindeeinheiten missing from the Getränke-bearbeiten-Karte).
      clearSettingsCache();
