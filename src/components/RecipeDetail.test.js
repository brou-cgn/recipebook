...
describe('RecipeDetail - Nutrition recalc icon', () => {
  ...
  test('shows "Nährwerte rekalkulieren" icon instead of filled icon when any ingredientID has recalc=true', async () => {
    const customLists = require('../utils/customLists');
    ...
    try {
      ...
      await waitFor(() => {
        expect(screen.getByLabelText('Nährwerte rekalkulieren')).toBeInTheDocument();
