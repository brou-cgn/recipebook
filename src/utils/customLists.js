...
// Shared style + step-boundary rules for the "zubereitung" field - must stay
// in sync with the identical constants in functions/index.js (extracted there
// so a future style change only needs updating in two files, not three: this
// client copy plus functions/index.js, which also reuses these two rules for
// its separate rephraseRecipeSteps prompt).
...
export const DEFAULT_AI_RECIPE_PROMPT = `Analysiere dieses Rezeptbild und extrahiere alle Informationen als strukturiertes JSON. Extrahiere nur das Rezept, ignoriere Kommentare, Likes, UI‑Text. Erfinde keine Zutaten/Mengen/Temperaturen/Zubereitungsschritte.
