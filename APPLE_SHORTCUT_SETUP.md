...
**[Kurzbefehl herunterladen](https://www.icloud.com/shortcuts/a9f137c8c6a04a97a8c611d3181d73fd)**
...
**Mehrere Fotos in den Body einsetzen:** Der native Array-Feld-Editor von „Inhalt von URL laden" erlaubt nur das manuelle Hinzufügen einzelner Elemente – er kann keine ganze Listen-Variable (z. B. die oben gesammelte Fotos-Liste) in einem Schritt als Array-Inhalt übernehmen. Shortcuts hat auch keine eigene „JSON kodieren"-Aktion, daher muss der JSON-Array-String von Hand gebaut werden (unproblematisch, da Base64-Text nie Anführungszeichen oder Backslashes enthält):

1. Aktion **„Text kombinieren"** auf die Fotos-Liste anwenden, Trennzeichen „Benutzerdefiniert" = `","` (Anführungszeichen, Komma, Anführungszeichen).
2. Danach eine **„Text"**-Aktion: `["` eintippen, das Ergebnis aus Schritt 1 einfügen, `"]` anhängen. Ergebnis: `["<Base64...>","<Base64...>"]`.
3. `images`-Feld im Body als Typ **Text** (nicht Array) anlegen und als Wert das Ergebnis aus Schritt 2 einsetzen.

Der Endpoint erkennt einen JSON-kodierten String in `images` automatisch und parst ihn serverseitig zu einem Array.
