...
test('keeps openfoodfacts confidence and ignores invalid ai confidence values', async () => {
  const originalFetch = global.fetch;

  createUtilsStub = () => ({
    parseIngredientForNutrition: (ingredient) => {
      if (ingredient.includes('Reis')) {
        return {amountG: 100, name: 'Rice', searchName: 'rice'};
      }
      return {amountG: 100, name: 'Mystery', searchName: 'mystery'};
    },
    normalizeIngredientWithGemini: async () => null,
    estimateNutritionWithGemini: async (_ingredient, parsed) => ({
      per100g: {
        kalorien: 50,
        protein: 1,
        fett: 0.5,
  ...
  const response = await wrappedFunction({
    auth: {uid: 'user-1'},
    data: {
      ingredients: ['100 g Reis', '100 g Mystery'],
      portionen: 1,
    },
  });
  ...
  assert.deepEqual(response.calcPer100g, {
    ...
    ballaststoffe: 0.7,
    salz: 0.05,
    confidence: {openFoodFacts: 0.5, ki: null},
  });
