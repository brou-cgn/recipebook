...
test('DRINK_WEIGHTS enthaelt alle erwarteten Kategorien', () => {
  const expected = ['bier', 'wein', 'softdrinks', 'spirituosen', 'kaffee', 'tee', 'wasser'];
...
test('calculate verteilt Gesamtgetraenkebedarf proportional per DRINK_WEIGHTS (Issue-Beispiel)', () => {
  ...
  // Kategorien: bier (0.26), softdrinks (0.26, 2x Drinks), wein (0.15)
  // Summe = 0.67
  ...
  const sumWeights = DRINK_WEIGHTS.bier.basis + DRINK_WEIGHTS.softdrinks.basis + DRINK_WEIGHTS.wein.basis;
  ...
  const expectedBier = Math.round((totalBeverage * DRINK_WEIGHTS.bier.basis / sumWeights) * 100) / 100;
...
test('calculate liefert Gesamtgetraenkebedarf = totalBeverage fuer alle DRINK_WEIGHTS-Kategorien', () => {
  // Mit allen sieben DRINK_WEIGHTS-Kategorien sollte die Summe = totalBeverage sein.
  const result = _internal.calculate(
      {
        eventName: 'Test',
        durationHours: 4,
        ...
        season: 'uebergang',
        eventType: 'familienfeier',
        categories: ['bier', 'wein', 'softdrinks', 'spirituosen', 'kaffee', 'tee', 'wasser'],
        customDrinkIds: [],
        pufferProzent: 0,
      },
      require('./drinkRates').DEFAULT_RATES,
